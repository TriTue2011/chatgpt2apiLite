...
class AccountService:
    ...
    @staticmethod
    def _load_accounts_from_env() -> dict[str, dict]:
        """Load accounts from CHATGPT_TOKEN_1..N environment variables.

        Inspired by Gemini-FastAPI's extract_gemini_clients_env() pattern.
        """
        accounts: dict[str, dict] = {}
        for key, value in os.environ.items():
            match = re.match(r'^CHATGPT_TOKEN_(\d+)$', key)
            if match and value.strip():
                token = value.strip()
                accounts[token] = {
                    "access_token": token,
                    "name": f"Account {match.group(1)}",
                    "source": "env",
                }
        return accounts

    def _load_accounts(self) -> dict[str, dict]:
        # Load accounts from storage backend (e.g., accounts.json)
        ...
        # Merge env-based accounts (env accounts take precedence)
        env_accounts = self._load_accounts_from_env()
        stored.update(env_accounts)
        return {token: acc for token, acc in stored.items() if acc}
